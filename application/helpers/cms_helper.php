...
function get_lang_menu ($array, $lang_code)
{
    ...
    foreach ($array as $item) {
        ...
        if($CI->uri->segment(1) == 'property')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('property/'.$CI->uri->segment(2).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('property/'.$CI->uri->segment(2).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        else if($CI->uri->segment(1) == 'showroom')
        {
            if($active)
            {
                $str.='<li class="active">'.anchor('showroom/'.$CI->uri->segment(2).'/'.$item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor('showroom/'.$CI->uri->segment(2).'/'.$item['code'], $item['code']).'</li>';
            }
        }
        ...
        else
        {
            if($active)
            {
                $str.='<li class="active">'.anchor($item['code'], $item['code']).'</li>';
            }
            else
            {
                $str.='<li>'.anchor($item['code'], $item['code']).'</li>';
            }
        }
    }
    $str.='</ul>';
    ...
    function get_ol ($array, $child = FALSE)
    {
    	...
    	if (count($array)) {
    		...
    		foreach ($array as $item) {
    			...
    			$str .= '<div class="" alt="'.$item['id'].'" >#'.$item['id'].'&nbsp;&nbsp;&nbsp;' . $item['option'] .'&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-'.$item['color'].'">'.$item['type'].'</span>&nbsp;&nbsp;'.$visible.'&nbsp;&nbsp;'.$locked.'&nbsp;&nbsp;'.$frontend.'<span class="pull-right">
